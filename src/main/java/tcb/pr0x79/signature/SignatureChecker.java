...
import tcb.pr0x79.ClassRelationResolver;
...
public class SignatureChecker {
	 ...
	 * @param requirementParameters The requirement's formal symbol parameters
	 * @param symbolParameters      The symbol's formal symbol parameters
	 * @param checkArgs             Whether symbol arguments should be checked
	 * @return
	 * @throws TypeVariableResolvingException Thrown if a symbol variable cannot be resolved
	...
	private boolean check(TypeSymbol requirement, TypeSymbol symbol,
		...
		if (requirement.isVariable() && symbol.isVariable()) {
			...
			if (resolvedRequirementParameter == null) {
				throw new TypeVariableResolvingException(String.format("Requirement symbol variable %s could not be resolved", requirement.getName()), requirement.getAsVariable(), true);
			}
			FormalTypeParameterSymbol resolvedSymbolParameter = symbolParameters.get(symbol.getName());
			if (resolvedSymbolParameter == null) {
				throw new TypeVariableResolvingException(String.format("Symbol symbol variable %s could not be resolved", symbol.getName()), symbol.getAsVariable(), false);
			}
			...
			switch (wildcard) {
				...
				case SignatureVisitor.SUPER:
					...
					break;
				default:
					throw new IllegalStateException("Unexpected value: " + wildcard);
			}
	...
	public static class TypeVariableResolvingException extends Exception {
		...
		/**
		 * The symbol variable that could not be resolved
		 *
		 * @return
		 */
