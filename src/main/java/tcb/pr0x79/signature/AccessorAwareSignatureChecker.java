...
public final class AccessorAwareSignatureChecker extends SignatureChecker {
	private final Accessors accessors;
	private final ClassLocatorRegistry locators;
	private final boolean bidirectional;

	public AccessorAwareSignatureChecker(ClassHierarchy hierarchy, ClassLoader loader, Accessors accessors, ClassLocatorRegistry locators, boolean bidirectional) {
	...
	protected boolean isCompatible(String requirementCls, String symbolCls, boolean includeInterfaces, boolean isSuper, boolean isKnownDowncast) {
		...
		if (this.bidirectional) {
			...
			return symbolAccessor != null && BytecodeInstrumentation.isIdentifiedClass(symbolAccessor, requirementCls, flags -> this.locators.getClass(this.loader, requirementCls, flags));
		} else {
			if (isSuper) {
