...
public class ClassRelationResolver {
	...
	public boolean traverseHierarchy(String clsName, Traverser traverser, boolean traverseInterfaces) {
		...
		ClassHierarchy.ClassData cls = this.hierarchy.getClass(this.loader, clsName, (n, f) -> {
			node[0] = n;
			node[1] = f;
		});
	...
	private boolean traverseHierarchy(ClassHierarchy.ClassData cls, ClassNode clsNode, int flags, Traverser traverser, boolean traverseInterfaces, Set<String> visitedItfs) {
		String type = cls.name;
		ClassHierarchy.ClassData info = cls;
		...
		while (!"java/lang/Object".equals(type)) {
			...
			if (traverseInterfaces) {
				...
				for (String itf : itfs) {
					...
					ClassHierarchy.ClassData itfData = this.hierarchy.getClass(this.loader, itf, (n, f) -> {
						itfNode[0] = n;
						itfNode[1] = f;
					});
			...
			if (type != null) {
				info = this.hierarchy.getClass(this.loader, type, (n, f) -> {
					node[0] = n;
					node[1] = f;
				});
	...
	public interface Traverser {
		 ...
		 */
		boolean traverse(String cls, boolean itf, ClassNode clsNode, int flags);
	}
}
